...
bool RHReliableDatagram::sendtoWait(uint8_t* buf, uint8_t len, uint8_t address)
{
	...
	while (retries++ <= _retries)
	{
		...
		#else
// To Do : Add a constant to the following formula in order to have a minimal delay
...
//	waitPacketSent();
	int16_t rssi = this->_driver.lastRssi();
	sendto((uint8_t *) &rssi, sizeof(rssi), from);
	waitPacketSent();
}
