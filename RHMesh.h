...
// Timeout for address resolution in ms
...
/// \par Route Discovery
///
/// When a RHMesh mesh node is initialized, it does not know any routes to any other nodes 
...
/// destination node in the routing table. If not, it will initiate 'Route Discovery'.
...
/// and therefore ignores it. This prevents broadcast storms.
/// When a node receives a RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_REQUEST it can use the list of 
/// nodes already visited to deduce routes back towards the originating (requesting node). 
...
/// RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_RESPONSE together ensure that the original requester and all 
/// the intermediate nodes know how to route to the source and destination nodes and every node along the path.
///
/// Note that there is a race condition here that can effect routing on multi path routes. For example, 
...
/// (say due to a lost route or no acknowledgment from the next hop), it replies to the 
/// originator with a unicast MeshRouteFailureMessage RH_MESH_MESSAGE_TYPE_ROUTE_FAILURE message. 
/// Intermediate nodes (on the way back to the originator)
...
/// which is beyond or at the limits of some Arduino models and other processors. Programs 
/// with additional software besides basic RHMesh programs may well require even more. If you have insufficient
/// SRAM for your program, it may result in failure to run, or weird crashes and other hard to trace behavior.
...
///
/// \par Performance
/// This class (in the interests of simple implementation and low memory use) does not have
/// message queuing. This means that only one message at a time can be handled. Message transmission 
...
public:
    ...
    typedef struct
    {
		...
		uint8_t             route[RH_MESH_MAX_MESSAGE_LEN - 2]; ///< List of node addresses visited so far. Length is implicit
	} MeshRouteDiscoveryMessage;
    ...
    /// Sends a message to the destination node. Initializes the RHRouter message header 
    ...
    /// Then waits for an acknowledgment from the next hop 
    ...
    /// addressed to other nodes and delivers any messages addressed to this node.
    /// If there is a valid application layer message available for this node (or RH_BROADCAST_ADDRESS), 
    /// send an acknowledgment to the last hop
    ...
    /// message available for this node or the timeout expires. 
    /// \param[in] buf Location to copy the received message
    /// \param[in,out] len Available space in buf. Set to the actual number of octets copied.
...
protected:
    ...
    /// Tests if the given address of length addresslen is identical to the
    ...
    /// \param [in] address Address of the physical address being tested
    /// \param [in] addresslen Length of the address in bytes
