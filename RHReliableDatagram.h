...
// The acknowledgment bit in the FLAGS
...
/// an acknowledgment is automatically sent to the sender.
///
...
/// transmitted again.  If no acknowledgment is received after several
/// retries, the transmissions is deemed to have failed.
/// No contention for media is detected.
/// This will be recognized as "pure ALOHA". 
...
/// acknowledgment) will be ignored. Your sketch will be unresponsive to new messages 
/// until an acknowledgment is received or the retries are exhausted. 
...
/// to process the acknowledgment. Best practice is to use the same processors (and
...
public:
    ...
    /// time of the acknowledgment (preamble+6 octets) plus the latency/poll time of the receiver. 
    ...
    /// Send the message (with retries) and waits for an ack. Returns true if an acknowledgment is received.
    ...
    /// \return true if the message was transmitted and an acknowledgment was received.
    bool sendtoWait(uint8_t* buf, uint8_t len, uint8_t address);

    /// If there is a valid message available for this node, send an acknowledgment to the SRC
    ...
    /// \return The number of retransmissions since initialization.
