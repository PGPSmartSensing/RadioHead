...
uint8_t RHMesh::sendtoWait(uint8_t* buf, uint8_t len, uint8_t address, uint8_t flags)
{
	...
	// Now we have a route. Lets construct an application layer message and send it via that route
...
uint8_t RHMesh::route(RoutedMessage* message, uint8_t messageLen)
{
	...
	{
		// Can't deliver to the next hop. Delete the route
...
// To Do : Execute the following only for continuously powered devices.
		else if (_dest == RH_BROADCAST_ADDRESS && tmpMessageLen > 1 && p->msgType == RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_REQUEST)
		{
			...
			// Get implicit number of already visited nodes
			...
			// Hasn't been passed to us yet, record routes back to the earlier nodes
			addRouteTo(_source, headerFrom()); // The originator of the discovery request
			...
			{
				// It's for someone else, rebroadcast it, after adding ourselves to the list
