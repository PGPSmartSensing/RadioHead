...
#define RH_ROUTING_TABLE_SIZE 16
...
/// the HOPS filed in the header is incremented. If a message is received for routing to another node 
...
/// (i.e. the nodes don't move around), and for which the 
...
/// and are handled automatically by the RH hardware.
...
/// - 1 octet DEST, the destination node address (i.e. the address of the final 
///   destination node for this message)
/// - 1 octet SOURCE, the source node address (i.e. the address of the originating node that first sent 
///   the message).
...
/// - 1 octet FLAGS, a bit mask for use by subclasses. Not used by RHRouter.
...
public:
    ...
    /// \param [in] state : The state of the route. Defaults to Valid
    ...
    /// message to the next hop. Waits for an acknowledgment from the next hop 
    ...
    ///           (usually because it does not acknowledge due to being off the air or out of range
    ...
    /// \return true if a valid message was received for this node copied to buf
...
protected:
    ...
    /// Lets subclasses peek at messages going past before routing or local delivery.
    ...
    /// Deletes a specific route entry from the routing table
