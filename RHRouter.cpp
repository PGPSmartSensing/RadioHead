...
void RHRouter::clearRoutingTable()
{
	...
	for (i = 0; i < RH_ROUTING_TABLE_SIZE; i++)
	{
		_routes[i].dest		= 0x00;		///< Destination node address
		_routes[i].next_hop = 0x00;		///< Next Hop to destination
...
//		|| (_thisAddress == 4 && (_from == 3 || _from == 5))
//		|| (_thisAddress == 5 && _from == 4)
